...
integer :: p0, p1
    ...
    do iter = 1,iter_max
		do i = 1,(iter_IR-1)
            ...
            ts_g_L(i) = (ts_L0(i+1)-ts_L0(i))/ts_L0(i)/ts_dt(i)
        enddo
        ts_g_k(iter_IR) = 0d0
        ts_g_L(iter_IR) = 0d0
        ...
        ts_k1(1) = k0
        do i = 2,iter_IR
            k = ts_k0(i-1)+ts_a_dk(i)*ts_dt(i-1)
        ...
        sup = max(maxval(abs(ts_I1-ts_I0)),                 &
                  ...
                  maxval(abs(B_N*ts_g_N1-B_N*ts_g_N0)),     &
                  maxval(abs(b_h*ts_g_h1-b_h*ts_g_h0)))                 
        ...
        ts_g_h0 = ts_g_h0+rho_ts*(ts_g_h1-ts_g_h0)
...
real(8) :: x(3), fvec(3)
    ...
    x(1) = L1; x(2) = epsilon_N1; x(3) = epsilon_I1
    call fsolve(RD_Equation,3,x,fvec,TOL,flag) 

    ts_g_N0 = g_N; ts_g_N1 = g_N
    ts_g_I0 = g_I; ts_g_I1 = g_I
    ts_g_h0 = g_h; ts_g_h1 = g_h
    ts_L0 = L; ts_L1 = L
...
real(8) :: gamma_I, d_Eta_I, d_Gamma_I, d_I 

    if (x(1) < 0d0 .or. x(2) < 0d0 .or. x(3) < 0d0 .or.      &
        x(1)+x(2)+x(3) > 1d0) then 
        fvec = -1d6 
        return 
    endif
    ...
    g_h = 1d0/mu_hH*(1d0-(L+epsilon_N+epsilon_I))**alpha
    g = B_N*g_N+b_h*g_h

    gamma_I = exp(B_N*(1d0-I_tilde))
    d_Eta_I = 1d0/I_tilde/(sigma-1d0)
    d_Gamma_I = B_N/(exp(B_N*(1d0-I_tilde)*(sigma-1d0))-1d0)
    d_I = s_K/(sigma-1d0)*d_Eta_I-(1d0-s_K/(sigma-1d0))*d_Gamma_I
    ...
    fvec(3) = b_h/mu_h*(1d0-(1d0-alpha)*(L+epsilon_N+epsilon_I))        &
                      *(1d0-(L+epsilon_N+epsilon_I))**(alpha-1d0)       &
            - rr+B_N*g_N+d_I*(g_I-g_N) 
...
real(8) :: x(3), fvec(3)
    ...
    if (rho+delta > A) then 
        do i = 1,NI
            x(1) = 0.1d0*real(i,8); x(2) = 1d0/3d0; x(3) = 1d0/8d0
            call fsolve(BGP_Equation,3,x,fvec,TOL,flag)
            if (flag == 1) exit
        enddo 
...
real(8), intent(out) :: fvec(n)
    ...
    if (I_tilde < 0d0 .or. I_tilde > 1d0 .or.           &
        L < 0d0 .or. epsilon_N < 0d0 .or.               &
        L+epsilon_N+epsilon_I > 1d0) then
        fvec = -1d6 
        return 
    endif
    ...
    g_N = 1d0/mu_N*epsilon_N**lambda
    g_I = 1d0/mu_I*epsilon_I**lambda
    g_h = 1d0/mu_h*(1d0-L-epsilon_N-epsilon_I)**alpha
    ...
    fvec(3) = b_h/mu_h*(1d0-(1d0-alpha)*(L+epsilon_N+epsilon_I))        &
                      *(1d0-(L+epsilon_N+epsilon_I))**(alpha-1d0)-rr+B_N*g_N
